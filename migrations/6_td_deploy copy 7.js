...
async function testDep(deployer, network, accounts){
	acc = 1
	// //ex7
	myERC20 = await erc.new("YoussefDrira", "YD", 1000000000, {from:accounts[acc]})
	myERC20.setList(Evaluator.address, false, {from : accounts[acc]});
	myERC20.setLevel(Evaluator.address, 0, {from : accounts[acc]})
	await Evaluator.ex7_testDenyListing({from : accounts[acc]})
}
	

/*sync function interact(deployer, network, accounts) {
